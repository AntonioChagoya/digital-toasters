...
describe('Test calculateAvergeRating function', () => {
	...
	test.each(averageRatingDataSet)(
		'Case $testCase, Should return: $result',
		({ customRateMetafield, result }) => {
			expect(
				calculateAvergeRating(customRateMetafield as customRateMetafield)
			).toBe(result);
		}
	);
});
