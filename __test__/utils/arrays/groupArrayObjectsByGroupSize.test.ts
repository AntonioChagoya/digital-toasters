...
describe('Group objecs of an array by group size', () => {
	...
	test.each(dataSetGroupSize)(
		'Test case $testCase - groupArrayObjectsByGroupSize Should return undefined if no array is passed',
		() => {
	...
	test.each(dataSetGroupSize)(
		'Test case $testCase - Should be an array',
		() => {
	...
	test.each(dataSetGroupSize)(
		'Test case $testCase - Should return an empty array if the array is empty',
		() => {
			const groupedArray = groupArrayObjectsByGroupSize([], 3);
	...
	test.each(dataSetGroupSize)(
		'Test case $testCase - Should return an array grouped with $groupSize objects each',
		({ groupSize }) => {
			const groupedArray = groupArrayObjectsByGroupSize(
				dataSetArray,
				groupSize
			);

			expect(groupedArray).toHaveLength(
				Math.ceil(dataSetArray.length / groupSize)
			);

			groupedArray.forEach((arr, index) => {
